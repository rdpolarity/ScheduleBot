...
async function updateschedules() {
  ...
  for (let i = 0; i < teamtable.length; i++) {
    let scheduleChannel = client.channels.find(
      channel => channel.id === teamtable[i].channel
    );
...
const sqlmatch = (team, date) =>
  `SELECT * FROM \`${team}\` WHERE \`date\` = '${date}'`;
...
const orderTableByDate = async object => {
  let sorted = object.sort((a, b) => {
    return moment(`${a.Date} ${a.Start}`, "DD/MM/YYYY h:mmA").diff(
      moment(`${b.Date} ${b.Start}`, "DD/MM/YYYY h:mmA")
    );
  });
  return sorted;
};
...
const addtable = async (name, id) => {
  ...
  if (!isTable) {
    sql =
      "CREATE TABLE `" +
      name +
      "` (`ID` int(11) NOT NULL AUTO_INCREMENT,`Opponent` varchar(50) NOT NULL,`Type` varchar(50) NOT NULL,`Date` varchar(50) NOT NULL,`Start` varchar(50) NOT NULL,`End` varchar(50) NOT NULL,PRIMARY KEY (`ID`)) ENGINE=MyISAM AUTO_INCREMENT=5 DEFAULT CHARSET=latin1";
    await query(sql);
    sql =
      "INSERT INTO `teams` (`team`, `channel`) VALUES ('" +
      name +
      "', '" +
      id +
      "')";
    ...
    return codeBlock(`[${name} Already Exists!]`);
  }
};
...
const removetable = async name => {
  ...
  if (isTable) {
    ...
    return codeBlock(`[${name} Does not exist!]`);
  }
};
...
const editTable = async (name, newname, newID) => {
  ...
  updateschedules();
  return codeBlock(
    `[${name}] has been updated to [${newname}] with the ID [${newID}]`
  );
};
...
async function getDay(team, date) {
  ...
  if (matches.length != 0) {
    for (let i = 0; i < matches.length; i++) {
      ...
      reply.push(
        `${centerText(`{${matches[i].Start}} - {${matches[i].End}}`, gap, "-")}`
      );
      reply.push(
        `${centerText(`#${matches[i].Type} ${matches[i].Opponent}`, gap, " ")}`
      );
    }
  } else {
    reply.push(centerText("", gap, "#"));
  }
...
async function getWeek(team) {
  ...
  for (let i = day; i <= day + 6; i++) {
    ...
    if (i == 7) {
      reply.push(centerText(`**[NEXT WEEK]**`, gap, "━"));
    }
  }
  const embed = new Discord.RichEmbed()
    .setAuthor(
      `${team.toUpperCase()} schedule in ${momentz()
        .tz("Australia/Sydney")
        .format("z")}`
    )
    .setColor("#ff0000")
...
const help = msg => {
  ...
  for (let i in cmds) {
    ...
    if (
      roles.some(role => {
        return cmds[i].permission.some(perm => {
...
//   await svgToImg
//     .from(svgstring)
//     .toPng({ path: path.join(__dirname, "temp.png") });
...
async function scrimAdd(team, opponent, type, date, start, end) {
  ...
  if (!moment(date, "DD/MM/YYYY", true).isValid())
    return codeBlock(`Date is incorrect, use format [DD/MM/YYYY]!`);
  if (!moment(start, "h:mmA", true).isValid())
    return codeBlock(`Start time is incorrect, use format [H:MM AM/PM]!`);
  if (!moment(end, "h:mmA", true).isValid())
    return codeBlock(`End time is incorrect, use format [H:MM AM/PM]!`);
  if (moment(start, "h:mmA").isAfter(moment(end, "h:mmA")))
    return codeBlock(`Start time cannot be before End time!`);
  let matches = await query(`SELECT * FROM ${team}`);
  let range = Moment.range(
    moment(`${date} ${start}`, "DD/MM/YYYY h:mmA"),
    moment(`${date} ${end}`, "DD/MM/YYYY h:mmA")
  );
  for (let i = 0; i < matches.length; i++) {
    let dbrange = Moment.range(
      moment(`${matches[i].Date} ${matches[i].Start}`, "DD/MM/YYYY h:mmA"),
      moment(`${matches[i].Date} ${matches[i].End}`, "DD/MM/YYYY h:mmA")
    );
    if (dbrange.overlaps(range))
      return codeBlock(`Date and time overlaps with existing match!`);
  }
  ...
  await query(sql);
  return codeBlock(
    `A [${type}] against [${opponent}] was added on [${date}] for [${team}]`
  );
}
...
const scrimRemove = async (team, id) => {
  let isTable = await checkTable(team);
  if (isTable == false) {
    return codeBlock(`[${team}] could not be found!`);
  }
  if (!/^[0-9]+$/.test(id)) return codeBlock(`[ID must be a number!]`);
  sql = `DELETE FROM \`${team}\` WHERE \`ID\` = ${id}`;
  let result = await query(sql);
  if (result.affectedRows == 0)
    return codeBlock(`[match ID ${id} could not be found!]`);
  updateschedules();
...
const addPerm = async (command, perm) => {
  ...
  try {
    cmds[command].permission.push(perm);
  } catch (err) {
    return codeBlock("[Command does not exist!]");
  }
...
const removePerm = async (command, perm) => {
  ...
  cmds = JSON.parse(cmds);
  try {
    var perms = cmds[command].permission;
  } catch (err) {
    return codeBlock("[Command does not exist!]");
  }
  ...
  if (msg.content[0] == "!" && msg.content.length > 1) {
    ...
    if (userCMD != cmd) {
      ...
      !msg.member.roles.some(role =>
        commands[cmd].permission.some(perm =>
          role.name.toLowerCase().includes(perm)
        )
      )
    ) {
      let permFormat = commands[cmd].permission
        .map(i => "``" + i + "``")
        .join(" or ");
      msg.reply(
        embed(
          "Permission Denied!",
          symbolAlert,
          `**!**${userCMD} is restricted to ${permFormat}`
        )
      );
      return true;
    } else if (args.length != commands[cmd].arguments.length) {
      let cmdFormat = commands[cmd].arguments
        .map(i => "``" + i + "``")
        .join(" ");
      msg.reply(
        embed(
...
const command = async m => {
  ...
  if (
    cget(m, "scrim-info", async args => {
      send(await getDay(...args));
    })
  )
    return;
  if (
    cget(m, "scrim-add", async args => {
      send(await scrimAdd(...args));
    })
  )
    return;
  if (
    cget(m, "scrim-remove", async args => {
      send(await scrimRemove(...args));
    })
  )
    return;
  if (
    cget(m, "help", async args => {
      send(help(m));
    })
  )
    return;
  if (
    cget(m, "info", async args => {
      send(info());
    })
  )
    return;
  ...
  // })) return;
  if (
    cget(m, "team-edit", async args => {
      send(await editTable(...args));
    })
  )
    return;
  if (
    cget(m, "team-add", async args => {
      send(await addtable(...args));
    })
  )
    return;
  if (
    cget(m, "team-remove", async args => {
      send(await removetable(...args));
    })
  )
    return;
  if (
    cget(m, "team-list", async args => {
      send(await teamlist());
    })
  )
    return;
  if (
    cget(m, "schedule", async args => {
      send(await getWeek(...args));
    })
  )
    return;
  if (
    cget(m, "perm-add", async args => {
      send(await addPerm(...args));
    })
  )
    return;
  if (
    cget(m, "perm-remove", async args => {
      send(await removePerm(...args));
    })
  )
    return;
  if (
    cget(m, "refresh", async args => {
...
client.on("message", async msg => {
  command(msg).catch(err =>
    msg.channel.send(
      codeBlock(`Error Occured, Report to #RDPolarity#5006\n [${err}]`)
    )
  );
});

if (debug) {
  client.login(config.debug);
} else {
...
// "graphic": {
//   "permission": ["founder"],
//   "arguments": ["template", "username"],
//   "description": "Renders out a twitter banner for the player"
// },
